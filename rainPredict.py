...
model = joblib.load('rainpredict_logistic_model.pkl')
...
min_temp = st.number_input("Min Temperature (°C)", -10.0, 50.0, 15.0)
max_temp = st.number_input("Max Temperature (°C)", -10.0, 50.0, 25.0)
rainfall = st.number_input("Rainfall (mm)", 0.0, 200.0, 2.0)
evaporation = st.number_input("Evaporation (mm)", 0.0, 100.0, 5.0)
sunshine = st.number_input("Sunshine (hrs)", 0.0, 15.0, 8.0)
...
cloud_9am = st.slider("Cloud Cover at 9am (0–8)", 0, 8, 4)
cloud_3pm = st.slider("Cloud Cover at 3pm (0–8)", 0, 8, 5)
temp_9am = st.number_input("Temperature at 9am (°C)", -10.0, 50.0, 18.0)
temp_3pm = st.number_input("Temperature at 3pm (°C)", -10.0, 50.0, 23.0)

# Wind info
wind_gust_dir = st.selectbox("Wind Gust Direction", ['E','ENE','ESE','N','NE','NNE','NNW','NW','S','SE','SSE','SSW','SW','W','WNW','WSW'])
wind_dir_9am = st.selectbox("Wind Direction at 9am", ['E','ENE','ESE','N','NE','NNE','NNW','NW','S','SE','SSE','SSW','SW','W','WNW','WSW'])
wind_dir_3pm = st.selectbox("Wind Direction at 3pm", ['E','ENE','ESE','N','NE','NNE','NNW','NW','S','SE','SSE','SSW','SW','W','WNW','WSW'])
...
for col in model.feature_names_in_:
    if col.startswith("Location_"):
        ...
        df_input[col] = 0  # for any missing column
...
if st.button("Predict Rain Tomorrow"):
    ...
    prediction_label = "Yes ☔" if prediction == 1 else "No 🌤️"
